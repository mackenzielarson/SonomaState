...
# Counts neighbors around a number in a matrix
...
# Draw the lines of the grid depending on the size
...
# Uses neighbor count to create new numbers
...
# Draws the board numbers depending on what neighbors there are
...
# Finds tile to remove based on where the user clicked
...
# Exposes cells below tile based on what their neighbors are, where the user clicked, and the position that the clicked tile was in
...
# Exposes all mines if one mine is clicked on
...
# Removes tiles completely based on what is clicked. If mine clicked, shows sad face to resemble loss or continues to be happy if won
def delete_tiles(tiles, cells, game_board_marker, rows, mid, win):
    while 2 > 1:
...
# Happy face is constantly on during minsweeper unless game lost
def happy_face(mid,win):

    y = Y_OFFSET
    smiley_image = Image(Point(mid, y-60), SMILEY_IMAGE)
    smiley_image.draw(win)

# Game lost if mine clicked
def game_lost(mid, win):
    y = Y_OFFSET
    sad_face = Image(Point(mid, y-60), LOSE_IMAGE)
    sad_face.draw(win)

# Main- where level selected and size of grid found. Order of operation
def main():
    ...
    delete_tiles(tiles, draw_grid, game_board_marker, rows, mid, win)
